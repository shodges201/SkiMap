...
//takes a list of open or closed trails and a value to set these trails to in the map that is also passed in
//valueInMap is either 1 or 0
function listOfTrails(list, valueInMap, map) {
    trailsList = [];
        ...
        for (let j = 0; j < trail.children.length; j++) {
            ...
            if (child.type === "text") {
                if (child.data.trim() && !notIncluded.includes(child.data.trim()) && child.data.trim() !== trailsList[trailsList.length - 1]) {
                    map.set(child.data.trim(), valueInMap);
                }
            }
        }
    }
